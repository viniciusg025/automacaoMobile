...
class RpcClient {
    ...
    async selectPage(appIdKey, pageIdKey, pageReadinessDetector) {
        await this._pageSelectionLock.acquire(toPageSelectionKey(appIdKey, pageIdKey), async () => {
            ...
            if (this.getTarget(appIdKey, pageIdKey)) {
                logger_1.default.debug(`Page '${pageIdKey}' is already selected for app '${appIdKey}'`);
                return;
            }
    ...
    _getPendingPageTargetDetails(appId, targetInfo) {
        const logInfo = (/** @type {string} */ message) => void logger_1.default.info(`Skipping 'Target.targetCreated' event ${message} for app '${appId}': ${JSON.stringify(targetInfo)}`);
